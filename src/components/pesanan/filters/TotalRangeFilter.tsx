...
export default function TotalRangeFilter({
  min,
  max,
  absoluteMin,
  absoluteMax,
  ...
  const trackStyle = {
    ...
    right: `${100 - ((max - absoluteMin) / (absoluteMax - absoluteMin)) * 100}%`
  };
  ...
  return (
    <div className="relative w-full">
      {/* Total Display */}
      <div className="flex justify-between mb-2 text-sm text-base-content">
        <span className="font-medium">{formatOrderPrice(min)}</span>
        <span className="font-medium">{formatOrderPrice(max)}</span>
      </div>
